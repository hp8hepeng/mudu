...
//定义地图图形的方法
    ...
    MapCharts.prototype = {
        ...
        init: function () {
            var _this = this;
            //请求svg地图
            _this.requestSvgMap('../../../JD/svg/mapid-' + _this.opts.mapId + '.svg');
        },
        /*
         * 设置svg地图
         * */
...
//          this.loopMapArea(_this.opts.index);
        ...
        addAreaMapInteraction: function () {
            ...
            $(document).click(function (e) {
                ...
                //排除svg地图内容区域
                if (target.nodeName == 'path' || target.nodeName == 'circle' || target.nodeName == 'rect' || target.nodeName == 'text' || target.nodeName == 'image' || target.nodeName == 'CANVAS') {
                } else {
                    //清除动画定时任务并重置地图样式
